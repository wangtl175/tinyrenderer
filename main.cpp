...
Vec3f barycentric(const Vec3f (&pts)[3], const Vec3f &P) {
    ...
    if (std::abs(u.z) < 1e-5) return {-1, 1, 1};
    return {1.f - (u.x + u.y) / u.z, u.y / u.z, u.x / u.z};
}

void triangle(Vec3f (&pts)[3], float *z_buffer, TGAImage &image, const TGAColor &color) {
